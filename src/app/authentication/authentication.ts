...
export class Authentication {
   ...
   * @author PSI-Enhancements
   */
  async userLoginHandler(reqObj: any) {
    this.spinner.show();
    try {
      const res = await this.authService.userLogin(reqObj);
      this.spinner.hide();
      if (!res.hasError) {
        this.authService.login(res.data.token, res.data);
   ...
   * @author PSI-Enhancements
   */
  setSessionOldNavigatorSite(token: any): Promise<void> {
      ...
      inputElementsArray.forEach((inputElement, index) => {
        ...
        if (inputElement && iconContainer && (!inputName || inputName === inputElementName)) {
          ...
          this.renderer.setStyle(
            ...
            'padding',
            `${topPadding} ${iconContainerWidth + extraPadding}px ${bottomPadding} ${leftPadding}`,
          );
        }
      });
    }
  ...
  /**
     ...
     * @author PSI-Enhancements
     */
  recalculatePaddingIfNeeded(field: string, value: any) {
