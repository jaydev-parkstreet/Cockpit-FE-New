...
@Component({
  ...
  imports: [CommonModule, Button, SearchBox],
...
export class SummaryTopBar {
  ...
  @Output() onClickAction: EventEmitter<{ action: Action }> = new EventEmitter<{
    action: Action;
  }>();
  ...
  updateSelectAllState(): void {
    const allSelected = Object.values(this.selectedFilters).every(
      (val) => val.length === this.filterList[val[0]?.id]?.length,
    );
    this.isAllItemsSelected = allSelected;
    this.isIndeterminate =
      !allSelected && Object.values(this.selectedFilters).some((val) => val.length > 0);
  }
  ...
  onSearchValueChange(value: string): void {
    // TO-DO: Handle the search value change
    console.log(value);
  }
}
